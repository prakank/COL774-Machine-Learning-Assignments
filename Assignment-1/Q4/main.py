...
def part5(X,Y,mu0,mu1,phi,sigma,sigma0,sigma1,A_lin,B_lin):
    ...
    term3 = np.dot( (np.dot(mu1.T, np.linalg.inv(sigma))), mu1) - np.dot( (np.dot(mu0.T, np.linalg.inv(sigma))), mu0)
    term3 = term3[0][0]
    ...
    title = "Quadratic Separator"
    filename = "Quadratic_Separator.jpg"
    graph_gda_quad(X,Y,term1,term2,-2*C - term3,A_lin,B_lin,title,filename)

...
path_train = os.path.join(BASE_DIR, "data", "mnist","train.csv")
path_test = os.path.join(BASE_DIR, "data", "mnist","test.csv")
binary = 0
part = 'b'

print("Files read start ...")
...
if(binary == 0):
    X_train_bin = train_array_x[(train_array_y==LAST_DIGIT).ravel() | (train_array_y==(LAST_DIGIT+1)).ravel()];
    Y_train_bin = train_array_y[(train_array_y==LAST_DIGIT).ravel() | (train_array_y==(LAST_DIGIT+1)).ravel()];
    Y_train_bin = -1.0 * (Y_train_bin==LAST_DIGIT) + 1.0 * (Y_train_bin==(LAST_DIGIT+1));

    X_test_bin = test_array_x[(test_array_y==LAST_DIGIT).ravel() | (test_array_y==(LAST_DIGIT+1)).ravel()];
    Y_test_bin = test_array_y[(test_array_y==LAST_DIGIT).ravel() | (test_array_y==(LAST_DIGIT+1)).ravel()];
    Y_test_bin = -1.0 * (Y_test_bin==LAST_DIGIT) + 1.0 * (Y_test_bin==(LAST_DIGIT+1));
